...
export default function BossScreen({ vals }: Props) {
  ...
  return (
    <View style={styles.container}>
      <RadialBackdrop width={width} height={height} cx={0.5} cy={0.32} colors={[bs.glow || '#c084fc', 'rgba(20,15,10,0.92)']} stops={[0, 0.72]} />
      ...
      <View style={[styles.hearts, compact && styles.heartsCompact]}>
      ...
      <Animated.View style={{ transform: [{ translateY: bob.interpolate({ inputRange: [0, 1], outputRange: [0, -8] }) }] }}>
        <RadialOrb size={compact ? 108 : 150} glow={bs.glow || '#c084fc'} body={bs.body || '#8B5CF6'} cx={0.38} cy={0.32} style={[styles.bossAvatar, compact && styles.bossAvatarCompact]}>
      ...
      <Text style={[styles.instruction, compact && styles.instructionCompact]}>Match the glowing weak point to strike!</Text>
      <View style={[styles.targetBox, compact && styles.targetBoxCompact, { shadowColor: bs.glow || '#c084fc' }]}>
        <Text style={styles.targetSym}>{bs.target}</Text>
      </View>

      <View style={[styles.options, compact && styles.optionsCompact]}>
        {options.map((o: any, i: number) => (
          <TouchableOpacity key={i} style={[styles.optBtn, compact && styles.optBtnCompact]} onPress={o.onTap} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  headerTextCompact: { top: 10, left: 16, right: undefined, alignItems: 'flex-start' },
  ...
  heartsCompact: { top: 72, left: 18, right: '54%' },
  ...
  bossAvatarCompact: { top: 98, left: 70 },
  ...
  instructionCompact: { top: 100, left: '42%', right: 16 },
  targetBox: {
  ...
  targetBoxCompact: { top: 130, left: '58%' },
  ...
  optionsCompact: { bottom: 22, left: '38%', right: 14 },
  optBtn: {
  ...
  optBtnCompact: { width: 64, height: 64, borderRadius: 17 },
